...
EXPOSED_JS_FUNCTIONS = pp.ZeroOrMore(
    pp.Suppress(
        ...
        + pp.Optional(pp.Or([pp.nestedExpr(), pp.Word(pp.printables, excludeChars=",")]) + pp.Literal(","))
    )
...
def _eel():
    ...
    page = page.replace("/** _start_geometry **/", "_start_geometry: %s," % _safe_json(start_geometry))
